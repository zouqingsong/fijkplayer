...
sigterm_handler(int sig)
{
    ...
    if(received_nb_signals > 3) {
        ...
        if (ret < 0) { /* Do nothing */ };
        // In library mode, don't call exit() - just return
        return;
    }
}
...
void term_init(void)
{
    ...
    // On iOS/tvOS/watchOS, do NOT install signal handlers.
    // They conflict with the debugger (SIGINT) and the system's signal handling,
    // causing crashes when running as a library inside an app.
#ifdef SIGPIPE
...
static void ffmpeg_cleanup(int ret)
{
    ...
    // Reset global counts so ffmpeg_main can be called again safely
    nb_input_files = 0;
    nb_output_files = 0;
    nb_filtergraphs = 0;
    nb_output_dumped = 0;
    ...
    // Reset additional state for library re-entry
